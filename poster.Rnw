...
%new column ------------------------------------------------------    
    ...
    The linear model above can be used only when the concentration of template molecules in sample is small (positive partitions contains very rarely more than 1 template particle). In situation when partitions are more saturated with template molecules, binomial regression is more precise. 
...
<<dube_bat, echo = FALSE, message=FALSE,fig.width=14.3,fig.height=15.3>>=
...
Black triangle marks real value of mean copies per partitions. Colour dots marks estimated values of mean copies per partitions. Positions of points are dodged only for purpose of readability.
    ...
    \begin{block}{Sample run}
    General linear models can be used for multple comparision of digital PCR experiments while still controlling type 1 errors. They can be also used to determine accurate confidence intervals for estimated mean copy numbers.
    \end{block}
    \vfill 
