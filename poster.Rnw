...
\begin{frame}
  \begin{columns}
    ...
    Generalized Linear Models (GLM) allows creating a linear model for data, where for example response variable has non-normal distributions (as binomially distributed positive partitions in dPCR experiments). We propose a linear model reflecting relationships in results of dPCR experiments:
    \begin{equation}
    \log{Y} = \beta^T X
    \end{equation}
