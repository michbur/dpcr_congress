...
@
    ...
    The proposed framework was evaluated in three Monte Carlo experiments (2000 times repetitions each) with accordingly 1000, 5000 (results not shown) and 10 000 partitions. During each repetition, a set of partitions was randomly generated~\shortcite{dube_mathematical_2008} with determined number of molecules ('Base number of molecules' on X-axis). The set was copied and a number of molecules ('Added number of molecules' on Y-axis) was added to randomly chosen partitions. Afterwards, two arrays were compared using proposed method. The p-value of test was saved.
