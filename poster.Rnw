...
\begin{frame}
  \begin{columns}
    ...
    The linear model above can be used only when the concentration of template molecules in sample is small (positive partitions contain very rarely more than 1 template particle). Higher concentrations requires binomial regression.
...
<<dube_bat, echo = FALSE, message=FALSE,fig.width=12,fig.height=11>>=
...
%new column ------------------------------------------------------    
    ...
    <<mc_figures, echo = FALSE, message=FALSE,fig.width=14.3,fig.height=24.5>>=
